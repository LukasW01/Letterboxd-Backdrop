...
const Alert: React.FC<Alert> = (props: Alert): ReactElement<string, string> | null => {
    ...
    return (
        show && props.text != '' ? (
            <div role="alert" className={`mx-4 animate-slide-in flex ${props.alert ? "text-red-500" : "text-green-500"}`}>
                <div className="gap-4 flex items-center justify-between">
                    <div>
                        <strong className={`font-medium ${props.alert ? "text-red-500" : "text-green-500"}`}>{props.alert ? "Error" : "Changes saved"}</strong>
